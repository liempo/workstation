...
|------|-------|---------------|
| SSH key auto-load (home-manager) | `com.liempo.ssh-add-keys` | `sh` (`/bin/sh` wrapper) |
...
Agents that start with `/bin/sh` show as `sh` in Settings (no stub `.app` / `AssociatedBundleIdentifiers`).
...
- A leftover `~/Applications/Workstation.app` from older generations is unused; remove it by hand if it remains after `drs`.
